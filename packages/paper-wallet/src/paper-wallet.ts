...
import { parseEthUrl } from "./eth-url-parser";
import { createPasswordEntryModal } from "./pin-modal/view";
import { createAccountScannerModal } from "./scan-modal/view";
...
export interface EncryptedPaperWallet {
  address: `0x${string}`;
...
export interface PlainPaperWallet {
  address: `0x${string}`;
  privateKey: string;
}
const tryParseEthUrl = (result: string) => {
  try {
    ...
    if (ethUrlResult.target_address) {
