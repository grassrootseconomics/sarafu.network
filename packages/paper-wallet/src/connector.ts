import type { Address, Chain, LocalAccount, ProviderConnectInfo } from "viem";
...
export function getPaperWalletConnector(options: PaperConnectorOptions) {
  return createConnector<boolean, Properties, StorageItem>((config: any) => ({
    id: "paperConnect",
    name: "PaperConnect",
    type: getPaperWalletConnector.type,
    ...
    async connect() {
      let wallet = PaperWallet.loadFromSessionStorage();
      if (wallet)
        return {
      ...
      wallet = await PaperWallet.fromQRCode();
...
getPaperWalletConnector.type = "paperConnect" as const;
