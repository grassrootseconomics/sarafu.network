...
export function ResponsiveTabs<T extends keyof Permissions = keyof Permissions>({
  tabs,
  ...
  return (
    <Tabs value={activeTab} onValueChange={onTabChange} className="w-full">
      <div className={containerClassName || "border-b border-gray-200 bg-white rounded-2xl overflow-hidden"}>
        {/* Desktop Tabs */}
        <TabsList
          className={
            tabsListClassName ||
          ...
          {tabs.map((tab) => {
            const trigger = (
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className={tabTriggerClassName || "px-6 py-2 text-sm font-medium"}
              >
                {tab.label}
              </TabsTrigger>
            );
      ...
      <Drawer open={drawerOpen} onOpenChange={setDrawerOpen} dismissible={true}>
            ...
            <div className="flex items-center gap-3">
              {currentTab && "icon" in currentTab && (
                <div className={`flex items-center justify-center w-8 h-8 rounded-full ${
                  drawerTriggerClassName?.includes("green") ? "bg-green-200" : "bg-gray-100"
                }`}>
                  <currentTab.icon className={`h-4 w-4 ${
                    drawerTriggerClassName?.includes("green") ? "text-green-700" : "text-gray-600"
                  }`} />
                </div>
              )}
              <div className="flex flex-col items-start">
                <span className={`font-semibold text-sm ${
                  drawerTriggerClassName?.includes("green") ? "text-green-900" : "text-gray-900"
                }`}>
                  {currentTab?.label}
                </span>
                <span className={`text-xs ${
                  drawerTriggerClassName?.includes("green") ? "text-green-600" : "text-gray-600"
                }`}>
        ...
        <DrawerContent
          className={drawerContentClassName || "p-2"}
        >
          <ScrollArea className="overflow-y-auto max-h-[85svh]">
            <div className="flex flex-col gap-1 p-2">
              {tabs.map((tab, index) => {
                const IconComponent = tab.icon;
                const isGreenTheme = drawerTriggerClassName?.includes("green");
                ...
                const button = (
                  <Button
                    ...
                    className={`justify-start h-auto py-4 px-3 ${
                      isGreenTheme ? "hover:bg-green-50" : "hover:bg-gray-50"
