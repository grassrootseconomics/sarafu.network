...
import { abi } from "@sarafu/contracts/abi/erc20-giftable-token/contract";
import { getFormattedValue, type TokenValue } from "@sarafu/core/units/token";
...
export const useExpiryPeriod = (address: `0x${string}`) => {
  ...
  const expiryDate = query.data
    ? new Date(Number(query.data) * 1000)
    : undefined;
  return {
    expires: expiryDate,
    hasExpired: expiryDate ? expiryDate < new Date() : false,
