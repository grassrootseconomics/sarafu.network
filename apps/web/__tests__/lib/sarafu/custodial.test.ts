...
import { deployERC20, deployDMR20, deployPool, trackOTX, getContractAddressFromTxHash, waitForDeployment, OTXType } from "@sarafu/contracts/sarafu/custodial";

// Mock the environment variables
vi.mock("@sarafu/contracts/env", () => ({
  env: {
...
vi.mock("@sarafu/contracts/chain", () => ({
  publicClient: {
...
describe("Deployment Tracking", () => {
  ...
  describe("waitForDeployment", () => {
    it("should successfully wait for deployment completion", async () => {
      ...
      const { publicClient } = await import("@sarafu/contracts/chain");
